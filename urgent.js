...
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('./client_main/client_menu/exif')
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetch, await, sleep, reSize } = require('./client_main/client_menu/myfunc')
   ...
   if (pairingCode && !conn.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +6281284760551")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Your WhatsApp bot number\nFor example: +6281284760551 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
    ...
    conn.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./Client")(conn, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
conn.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
console.log(chalk.green('🟨Welcome to ERZA-md'));
...
📃 Reason: Left
📔 Name: @${connName.split("@")[0]}
👥 Members: ${connmembers}th
🕒 Time: ${conntime} ${conndate}
...
const conndate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
let connName = num
connbody = ` *Selamat* 🎉 @${connName.split("@")[0]}, you have been *promoted* to *admin* 🥳`
   conn.sendMessage(anu.id,
 { text: connbody,
 contextInfo:{
 mentionedJid:[num],
...
const conndate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
let connName = num
connbody = `𝗢𝗼𝗽𝘀‼️ @${connName.split("@")[0]}, you have been *demoted* from *admin* 😬`
conn.sendMessage(anu.id,
 { text: connbody,
 contextInfo:{
 mentionedJid:[num],
