...
const { TelegraPh, UploadFileUgu, webp2mp4File, floNime } = require('./client_main/client_menu/uploader')
const { toAudio, toPTT, toVideo, ffmpeg, addExifAvatar } = require('./client_main/client_menu/converter')
const { smsg, getGroupAdmins, formatp, jam, formatDate, getTime, isUrl, await, sleep, clockString, msToDate, sort, toNumber, enumGetKey, runtime, fetchJson, getBuffer, json, delay, format, logic, generateProfilePicture, parseMention, getRandom, pickRandom, reSize } = require('./client_main/client_menu/myfunc')
let afk = require("./client_main/client_menu/afk");

const { fetchBuffer, buffergif } = require("./client_main/client_menu/myfunc2")
...
"previewType": "PHOTO",
"thumbnailUrl": ``,
"thumbnail": fs.readFileSync(`./client_main/thumb.jpg`),
"sourceUrl": `${link}`}}},
{ quoted: m})
}
...
await Client.sendMessage(from, {text: Ayushlod[i], edit: key });
        ...
        Client.sendPresenceUpdate(Ayushrecordinfinal, from)

        }
        
        if (autobio) {
            Client.updateProfileStatus(`Hey, future leaders! 🌟 Client-Md is here to inspire and lead, thanks to Ayush Botz, Inc. 🚀 ${runtime(process.uptime())} `).catch(_ => _)
        }
...
reply('Success in turning off all antilink in this group')
  ...
  break
            ...
            case 'clearsession': {
                ...
                fs.readdir("./client_main/session", async function(err, files) {
                    if (err) {
                    ...
                    await filteredArray.forEach(function(file) {
                        fs.unlinkSync(`./client_main/session/${file}`)
                    });
                    await sleep(2000)
            ...
            case 'join':
                try {
                    ...
                    await Client.groupAcceptInvite(result).then((res) => reply(json(res))).catch((err) => reply(json(err)))
                } catch {
            ...
            case 'setppbot':
                ...
                if (args[0] == 'full') {
                    ...
                    var memeg = await Client.updateProfilePicture(botNumber, {
                        url: medis
                    })
                    fs.unlinkSync(medis)
                    reply(mess.done)
            ...
            case 'block':
                ...
                await Client.updateBlockStatus(blockw, 'block').then((res) => reply(json(res))).catch((err) => reply(json(err)))
                break
            case 'unblock':
                if (!isCreator) return reply(mess.owner)
                let blockww = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '') + '@s.whatsapp.net'
                await Client.updateBlockStatus(blockww, 'unblock').then((res) => reply(json(res))).catch((err) => reply(json(err)))
                break
            case 'leave':
                if (!isCreator) return reply(mess.owner)
                if (!m.isGroup) return reply(mess.group)
            ...
            case 'kick':
                ...
                await Client.groupParticipantsUpdate(m.chat, [blockwww], 'remove').then((res) => reply(json(res))).catch((err) => reply(json(err)))
                break
            case 'add':
                if (!m.isGroup) return reply(mess.group)
                if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin)
                ...
                await Client.groupParticipantsUpdate(m.chat, [blockwwww], 'add').then((res) => reply(json(res))).catch((err) => reply(json(err)))
                break
            case 'promote':
                if (!m.isGroup) return reply(mess.group)
                if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin)
                ...
                await Client.groupParticipantsUpdate(m.chat, [blockwwwww], 'promote').then((res) => reply(json(res))).catch((err) => reply(json(err)))
                break
            case 'demote':
                if (!m.isGroup) return reply(mess.group)
                if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin)
                ...
                await Client.groupParticipantsUpdate(m.chat, [blockwwwwwa], 'demote').then((res) => reply(json(res))).catch((err) => reply(json(err)))
                break
            case 'setname':
            case 'setsubject':
                if (!m.isGroup) return reply(mess.group)
                ...
                if (!isBotAdmins) return reply(mess.botAdmin)
                if (!text) return 'Text ?'
                await Client.groupUpdateSubject(m.chat, text).then((res) => reply(mess.success)).catch((err) => reply(json(err)))
                break
            case 'setdesc':
            case 'setdesk':
                if (!m.isGroup) return reply(mess.group)
                ...
                if (!isBotAdmins) return reply(mess.botAdmin)
                if (!text) return 'Text ?'
                await Client.groupUpdateDescription(m.chat, text).then((res) => reply(mess.success)).catch((err) => reply(json(err)))
                break
            case 'setppgroup':
            case 'setppgrup':
            case 'setppgc':
                ...
                if (args[0] == 'full') {
                    ...
                    var memeg = await Client.updateProfilePicture(m.chat, {
                        url: medis
                    })
                    fs.unlinkSync(medis)
                    reply(mess.done)
 ...
 *Message : ${args.join(" ") ? args.join(" ") : 'no message'}*\n\n`
 ...
 break
            ...
            case 'grup':
                ...
                if (!isBotAdmins) return reply(mess.botAdmin)
                if (args[0] === 'close') {
                    await Client.groupSettingUpdate(m.chat, 'announcement').then((res) => reply(`♠️Success In Closing The Group️`)).catch((err) => reply(json(err)))
                } else if (args[0] === 'open') {
                    await Client.groupSettingUpdate(m.chat, 'not_announcement').then((res) => reply(`Success In Opening The Group 🕊️`)).catch((err) => reply(json(err)))
                } else {
            ...
            case 'editinfo':
                ...
                if (!isBotAdmins) return reply(mess.botAdmin)
                if (args[0] === 'open') {
                    await Client.groupSettingUpdate(m.chat, 'unlocked').then((res) => reply(`Successfully Opened Group Edit Info 🕊️`)).catch((err) => reply(json(err)))
                } else if (args[0] === 'close') {
                    await Client.groupSettingUpdate(m.chat, 'locked').then((res) => reply(`Successfully Closed Group Edit Info🕊️`)).catch((err) => reply(json(err)))
                } else {
      ...
      case "grouplink":
        ...
        await Client.sendMessage(m.chat, { video: { url: `https://media.tenor.com/hzWYhzhMTeEAAAPo/Client-useless.mp4` }, caption: 'I sent you the Group Link in personal message.\n Pls check.', gifPlayback: true }, { quoted: m });
        break
  ...
  *🌟 Repo:* https://github.com/AYUSH-PANDEY023/Client-Md \n
...
*❝ Dont forget to give a Star ⭐ to the repo.*`
        ...
        let buttonMessage = {
                 video:fs.readFileSync('./client_main/thumb.jpg'),gifPlayback:true,   
                    caption: scritxt,
...
break
            ...
            case 'toptt': {
                ...
                let {
                    toPTT
                } = require('./client_main/client_menu/converter')
...
break
      ...
      case 'qc': {
                const {
                    quote
                } = require('./client_main/client_menu/quote.js')
...
const Ayushplaymp3 = require('./client_main/client_menu/ytdl2')
...
const Ayushaudp3 = require('./client_main/client_menu/ytdl2')
...
const Ayushvidoh = require('./client_main/client_menu/ytdl2')
...
case 'chatgpt': case 'gpt':{
        ...
        const helptxt = `_*📍[Rules for Client Md usage]📍*_\n\n\n*>>>* use -support to get the Official group link in your dm.\n\n*--->* If you want to add Client-Md in your group the contact the owner by *-owner/-mods* \n\n*--->* Dont use wrong command, use the command given in the *-help* list \n\n* Dont spam the bot with commands if Client-Md is not responding, its means the maybe owner is offline or facing internet issue. \n\n*IF YOU DONT FOLLOW THE RULES THEN YOU WILL BE BANNED* 🚫 \n\n\n*©️ Ayush Bots inc* `

        Client.sendMessage(from, { video: { url: 'https://c.tenor.com/geMdtLCXZkAAAAPo/rules.mp4' }, gifPlayback: true, caption: helptxt }, { quoted: m })

        break
      case 'hii': case 'hi': case 'Hi':
       
        
        let txxt = `👋🏻 Hi *${pushname}*, i am  *Client-Md*📍\nA whatsapp bot created by: Ayush \n\n I don't have time for chit-chat Darling. Use command from *${prefix}help* list if you want me to do anything.`

        Client.sendMessage(m.chat, { video: { url: `https://c.tenor.com/4TLYvKWI2mgAAAPo/nakano-yotsuba-smile.mp4` }, caption: txxt, gifPlayback: true }, { quoted: m });
        break
      case "support":
      
        let tex = `📍My Developer's Group📍\n\n*🎇 𝐌𝐚𝐫𝐢𝐚 support group:🎇* *https://chat.whatsapp.com/FoS7pSPtfMqBuoireK4aAJ*`

        await Client.sendMessage(m.sender, { text: `${tex}` },);

        await Client.sendMessage(m.chat, { video: { url: `https://i.makeagif.com/media/3-25-2023/NO05ia.mp4` }, caption: 'I sent you the support Link in personal message.\n Pls check.', gifPlayback: true }, { quoted: m });
        break
      ...
      case "info":
        ...
        let ifx = `🌟『𝕄𝕒𝕣𝕚𝕒-𝕄𝕕 』🌟
*🌟Description:* A WhatsApp Bot With Rich  features based on Client
*🚦Uptime:* ${runtime(process.uptime())}
*🕸Version:* 1.2.0
*👤Owner:*  ${global.Owner}
*🎐supportgc:* ${global.support}\n
*Powered by Ayush*`
Client.sendMessage(m.chat, { video: { url: `https://media.tenor.com/VmwZnGeD0oEAAAPo/Client-naruse-shinmai-maou-no-testament.mp4` }, caption: ifx, gifPlayback: true }, { quoted: m });
        break
...
"eat 2 tablespoons of rice without any side dishes, if it's dragging you can drink",
         ...
         "say i love the bot owner Client through voice note",
...
const Clientarray= [
            ...
            const Clientselection = Clientarray[Math.floor(Math.random()*Clientarray.length)]
        
            Client.sendMessage(from,{video:{url:Clientselection},gifPlayback:true,caption:txt},{quoted:m})
...
case 'runtime': {
        ...
        Client.sendText(ownernumber + '@s.whatsapp.net', util.format(err), m)
